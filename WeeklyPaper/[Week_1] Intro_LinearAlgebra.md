...
##### 결국 강의가 좋았던 이유는
선형대수 대가의 insight를 배울 수 있다는게 가장 중요하다. 교수님이 한학기 강의 내내 강조하는 부분은 결국 `Vectorspace`에 대한 이해이다.
나처럼 공대생 관점에서 선형대수는 **데이터를 다루는 언어**이기도 하며 데이터에 대한 깊은 통찰력이 생긴다는 점이 중요하다.


##### 다시 돌아가서.. 결론은
>AI는 복잡한 모델을 학습시키는 것이고 이러한 네트워크의 파라미터와 데이터는 선형적일수 밖에 없기 때문에 모든 학습은 선형대수를 사용하지 않을 수가 없다.  
